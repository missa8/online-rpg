...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ name, reconnectId }) => {
    name = String(name || '').trim().slice(0, 30);
    if (!name) return socket.emit('error', { message: 'Nom invalide.' });

    // Reconnexion : restaure l'inventaire, les stats, les compétences
    if (reconnectId && state.players[reconnectId]) {
      const p = state.players[reconnectId];
      p.name     = name;
      ...
      syncGm();
      console.log(`[+] Joueur reconnecté : ${name}`);
      return;
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (myRole === 'gm') {
      ...
      Object.values(state.players).forEach(p =>
        p.socketId && io.to(p.socketId).emit('gm_status', { online: false })
      );
    } else if (myId && state.players[myId]) {
      console.log(`[-] Joueur déconnecté : ${state.players[myId].name} (inventaire conservé)`);
      state.players[myId].online = false;
      state.players[myId].socketId = null;
      syncGm();
    }
  });
});
